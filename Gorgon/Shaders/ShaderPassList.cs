		...
		internal void Add(ShaderTechnique technique)
		{
			try
			{
				...
				for (int i = 0; i < techniqueInfo.Passes; i++)
				{
					...
					// Update collection.
					if (_items.ContainsKey(newPass.Name))
						_items[newPass.Name] = newPass;
					else
						_items.Add(newPass.Name, newPass);
				}
			}
			...
			{
				throw new CannotRetrievePassesException(technique, ex);
			}
		}
		#endregion
