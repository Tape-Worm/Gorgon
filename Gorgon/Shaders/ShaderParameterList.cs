		...
		: Collection<ShaderParameter>
	{
		...
		internal void Add(Shader shader)
		{
			try
			{
				...
				{
					D3D.EffectHandle handle;					// Handle.
					D3D.ParameterDescription description;		// Description.
					...
					// Get the parameter.
					newParameter = new ShaderParameter(description.Name, handle, shader, i);
					...
					foreach (ShaderTechnique technique in shader.Techniques)
					{
						if (shader.D3DEffect.IsParameterUsed(handle, technique.D3DEffectHandle))
							technique.Parameters.Add(new ShaderParameter(description.Name, handle, shader, i));
					}
				}
			}
			...
			{
				throw new CannotRetrieveParametersException(shader.Name,ex);
			}
		}
		#endregion
