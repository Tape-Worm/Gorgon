...
namespace GorgonLibrary.Internal
{
	...
	public class VertexField
	{
		...
		{
			if (obj is VertexField)
			{
				VertexField left = (VertexField)obj;		// Comparison vertex field object.
				if ((left._context == _context) && (left._index == _index) && (left._offset == _offset) && (left._stream == _stream) && (left._type == _type))
					return true;
			}
		...
		/// <param name="context">The purpose of this field.</param>
		/// <param name="fieldType">Data type of the field.</param>
		internal VertexField(short stream,short fieldOffset, VertexFieldContext context, VertexFieldType fieldType) : this(stream,fieldOffset,context,fieldType,0)
		{
		}
		#endregion
	}
