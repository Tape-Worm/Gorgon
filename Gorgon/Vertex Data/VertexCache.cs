...
namespace GorgonLibrary.Internal
{
		...
		: IDataCache<V>, IDisposable, IDeviceStateObject		
	{
		...
		private BufferUsage _vertexBufferUsage;		// Usage flags for each buffer.
		...
		public VertexType VertexType
		{
			...
			{
				if (value == null)
					throw new InvalidVertexTypeException(null);
		...
		public VertexCache()
		{
			...
			_vertices = null;
			_compiled = false;
			_vertexBufferUsage = BufferUsage.Static | BufferUsage.WriteOnly;
			_verticesWritten = 0;
			_vertexOffset = 0;
        ...
        public void Update(bool isRestoring)
        {
			...
			if ((_vertexCount > 0) && (!_compiled) && (_vertices != null))
			{
				...
				GCHandle sourceArray = GCHandle.Alloc(_vertices, GCHandleType.Pinned);
				// Now copy data into the buffers.
				if (((_vertexOffset + _verticesWritten) < _vertexCount) && ((_vertexBufferUsage & BufferUsage.Dynamic) != 0) && (_vertexOffset > 0))
				{					
				...
				int vertexSize = (_verticesWritten * _vertexType.VertexSize(0));
				unsafe
				{
					byte* vertices = (byte *)Marshal.UnsafeAddrOfPinnedArrayElement(_vertices,0).ToPointer();	// Pointer for the buffer.
					byte* buffer = (byte *)_vertexBuffer.LockPointer.ToPointer();								// Vertex buffer pointer.
        ...
        public void Release()
        {
			if ((_vertexBuffer != null) && ((_vertexBuffer.BufferUsage & BufferUsage.Dynamic) != 0))
			{
				_vertexBuffer.Release();
				_compiled = false;
			}
