...
namespace GorgonLibrary.Internal
{
		...
		: IDataCache<T>, IDisposable, IDeviceStateObject
	{
		...
		private BufferUsage _indexBufferUsage;		// Buffer usage for index buffer.
		...
		public IndexCache()
		{
			_indexBufferUsage = BufferUsage.Static | BufferUsage.WriteOnly;
		...
		public void Update(bool isRestoring)
		{
			...
			if ((_indexCount > 0) && (!_compiled) && (_indices != null))
			{
				...
				GCHandle sourceArray = GCHandle.Alloc(_indices,GCHandleType.Pinned);
				// Copy the data into the index buffer.
				// Now copy data into the buffers.
				if (((_indexOffset + _indicesWritten) < _indexCount) && ((_indexBufferUsage & BufferUsage.Dynamic) != 0))
				{
				...
				if ((_indexBufferUsage & BufferUsage.Dynamic) != 0)
					_indexOffset += _indicesWritten;

				unsafe
				{
					if (_indexBufferType == IndexBufferType.Index16)
					{
						short* indices16 = (short*)Marshal.UnsafeAddrOfPinnedArrayElement(_indices, 0).ToPointer();	// Pointer to the array.
						short* buffer16 = (short*)_indexBuffer.LockPointer.ToPointer();								// Pointer to the index buffer.

						// Copy.
						for (int i = 0; i < _indicesWritten; i++)
						{
							*buffer16 = *indices16;
							buffer16++;
							indices16++;
					...
					{
						int* indices32 = (int*)Marshal.UnsafeAddrOfPinnedArrayElement(_indices, 0).ToPointer();	// Pointer to the array.
						int* buffer32 = (int*)_indexBuffer.LockPointer.ToPointer();								// Pointer to the index buffer.
		...
		public void Release()
		{
			...
			if (_indexBuffer != null)
			{
				if ((_indexBuffer.BufferUsage & BufferUsage.Dynamic) > 0)
				{
