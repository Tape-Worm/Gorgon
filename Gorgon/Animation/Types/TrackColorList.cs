...
{
	/// <summary>
	/// Object representing a list of color tracks.
	/// </summary>
	public class TrackColorList
		: TrackList<TrackColor>
	{
		#region Method.
		/// <summary>
		...
		public override TrackColor Create(string name)
		{
			TrackColor newTrack = null;       // New track.

			if (Contains(name))
				throw new DuplicateObjectException(name);

			newTrack = new TrackColor(Owner, name);
		...
		internal TrackColorList(Animation owner)
			: base(owner)
		{
		}
		#endregion
