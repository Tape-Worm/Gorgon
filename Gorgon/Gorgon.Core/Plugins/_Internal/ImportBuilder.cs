...
namespace System.ComponentModel.Composition.Registration
{
    internal sealed class ImportBuilder
    {
        ...
        public ImportBuilder AsContractType<T>() => AsContractType(typeof(T));
        ...
        internal void BuildAttributes(Type type, ref List<Attribute> attributes)
        {
            ...
            bool asMany = (!_asManySpecified) ? type != _stringType && typeof(IEnumerable).IsAssignableFrom(type) : _asMany;
            if (!asMany)
            {
