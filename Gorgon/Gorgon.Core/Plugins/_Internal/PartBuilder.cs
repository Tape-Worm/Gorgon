...
namespace System.ComponentModel.Composition.Registration
{
    ...
    {
        private static readonly List<Attribute> _importingConstructorList = new List<Attribute>() { new ImportingConstructorAttribute() };
        private static readonly Type _exportAttributeType = typeof(ExportAttribute);
        ...
        internal Predicate<Type> SelectType
        {
            get; private set;
        }
        ...
        public PartBuilder Export() => Export(null);
        ...
        public PartBuilder Export<T>() => Export<T>(null);
        ...
        public PartBuilder SelectConstructor(Func<ConstructorInfo[], ConstructorInfo> constructorFilter) => SelectConstructor(constructorFilter, null);
        ...
        public PartBuilder ExportInterfaces(Predicate<Type> interfaceFilter) => ExportInterfaces(interfaceFilter, null);

        public PartBuilder ExportInterfaces() => ExportInterfaces(t => true, null);
        ...
        private static bool MemberHasExportMetadata(MemberInfo member)
        {
            foreach (Attribute attr in member.GetCustomAttributes(typeof(Attribute), false))
            {
                ...
                else
                {
                    ...
                    if (attrType != _exportAttributeType && attrType.IsDefined(typeof(MetadataAttributeAttribute), true))
                    {
                        return true;
                    }
                }
        ...
        private static void ConfigureConstructorAttributes(ConstructorInfo constructorInfo, ref List<Tuple<object, List<Attribute>>> configuredMembers, Action<ParameterInfo, ImportBuilder> configureConstuctorImports)
        {
            ...
            // Make its attribute
            configuredMembers.Add(Tuple.Create((object)constructorInfo, _importingConstructorList));
