...
namespace GorgonLibrary.Graphics
{
	...
	public class Viewport
	{
		...
		#region Properties.
		/// <summary>
		/// Property to set or return whether the projection changed.
		/// </summary>
		internal bool ProjectionChanged
		{
			get
			{
		...
		public bool UseScissorTestClipping
		{
			get
			{
				if (Gorgon.Driver != default(Driver))
					return Gorgon.Driver.SupportScissorTesting;
				else
					return false;
			}
		}
		...
		public Matrix ProjectionMatrix
		{
			get
			{
				...
				{
					_orthoMatrix = Gorgon.Renderer.CreateOrthoMatrix(_viewPort.Left, _viewPort.Top, _viewPort.Width, _viewPort.Height);
		...
		public Rectangle ClippedDimensions
		{
			get
			{
				...
				if (Gorgon.Renderer.CurrentRenderTarget == null)
					return Rectangle.Empty;

				dimensions = _windowRect;
				targetDimensions = new Rectangle(0, 0, Gorgon.Renderer.CurrentRenderTarget.Width, Gorgon.Renderer.CurrentRenderTarget.Height);
		...
		public void Refresh()
		{
			...
			target = new Rectangle(0, 0, Gorgon.Renderer.CurrentRenderTarget.Width, Gorgon.Renderer.CurrentRenderTarget.Height);
