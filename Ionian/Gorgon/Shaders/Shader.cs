...
/*
		...
		{
#if DEBUG
			CompileShader(ShaderCompileOptions.Debug);
#else
			CompileShader(ShaderCompileOptions.OptimizationLevel3);			
#endif
		}
		...
		public void CompileShader(ShaderCompileOptions flags)
		{
			...
			try
			{
				...
				compiler = new D3D9.EffectCompiler(_shaderCode.ToString(), null, null, d3dflags, out errors);
		...
		/// </list>
		/// </remarks>		
		public ShaderFunction GetShaderFunction(string function, string shaderTarget)
		{
#if DEBUG
			return GetShaderFunction(function, shaderTarget, ShaderCompileOptions.Debug);
#else
			return GetShaderFunction(function, shaderTarget, ShaderCompileOptions.OptimizationLevel3);
#endif
		}
		...
		/// <param name="function">Name of the function.</param>
		/// <param name="shaderTarget">Target for the shader function.</param>
		/// <param name="options">Compilation options for the shader function.</param>
		/// <returns>The extracted function from the shader.</returns>
		/// <remarks>The shaderTarget parameter expects one of the following:
		/// <list type="table">
		///		<listheader><term>Target value</term><description>Description</description></listheader>
		...
		/// </remarks>		
		public ShaderFunction GetShaderFunction(string function, string shaderTarget, ShaderCompileOptions options)
		{
			...
			if ((_compiled == null) || (_compiled.Length == 0))
				throw new ShaderCannotCreateTextureShaderException("Compiled.");
			...
			try
			{
				...
				flags = D3D9.ShaderFlags.OptimizationLevel3;
#endif
				compiler = new D3D9.EffectCompiler(_compiled, null, null, flags, out errors);
				functionHandle = compiler.GetFunction(function);

				// Create code.
				byteCode = compiler.CompileShader(functionHandle, shaderTarget, flags, out errors);
