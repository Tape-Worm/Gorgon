...
{
	float4 sample = tex2D(vBlurSampler, vtx.texCoords);		// Our texel to blur.
	
	for (int i = 0; i < 13; i++)
	{
	   sample += tex2D(vBlurSampler, float2(vtx.texCoords.x + kernel[i] / blurAmount, vtx.texCoords.y));
	}
...
{
	float4 sample = tex2D(hBlurSampler, vtx.texCoords);		// Our texel to blur.
	
	for (int i = 0; i < 13; i++)
	{
	   sample += tex2D(hBlurSampler, float2(vtx.texCoords.x, vtx.texCoords.y + kernel[i] / blurAmount));
	}
