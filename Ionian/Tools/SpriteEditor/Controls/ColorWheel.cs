...
namespace GorgonLibrary.Graphics.Tools
{
	internal class ColorWheel : IDisposable
	{
		...
		private byte _alpha = 255;			// Alpha of the color.
		private bool _useAlpha = true;		// Flag to use alpha blending.
		...
		private const int COLOR_COUNT = 256 * 256;
		...
		private Rectangle _alphaRectangle;
		...
		public ColorWheel(Rectangle colorRectangle, Rectangle brightnessRectangle, Rectangle selectedColorRectangle, Rectangle alphaRectangle)
		{
			...
			using (GraphicsPath path = new GraphicsPath())
			{
				...
				this._alphaRectangle = alphaRectangle;
				...
				path.AddRectangle(new Rectangle(_alphaRectangle.Left, _alphaRectangle.Top - 10, _alphaRectangle.Width + 10, _alphaRectangle.Height + 20));
				...
				alphaPoint = new Point(alphaRectangle.Right, alphaRectangle.Top);
		...
		/// </summary>
		public Rectangle SelectedColorRectangle
		{
			get
			{
				return selectedColorRectangle;
			}
			set
			{
				this.selectedColorRectangle = value;
			}
		}
		...
		{
			ColorChangedEventArgs e = new ColorChangedEventArgs(RGB, HSV, _alpha);
			ColorChanged(this, e);
		}
		...
		public Color Color
		{
			get
			{
				return Color.FromArgb(_alpha, selectedColor);
			}
		}
		...
		public void Draw(System.Drawing.Graphics g, ColorHandler.HSV HSV)
		{
		...
		public void Draw(System.Drawing.Graphics g, ColorHandler.RGB RGB)
		{
		...
		public void Draw(System.Drawing.Graphics g, Point mousePoint)
		{
			...
			if (currentState == MouseState.MouseUp)
			{
				if (!mousePoint.IsEmpty)
				{
					...
					else if ((alphaRegion.IsVisible(mousePoint)) && (_useAlpha))
					{
			...
			switch (currentState)
			{
				...
				case MouseState.DragInAlpha:
					...
					newPoint = mousePoint;
					if (newPoint.Y < _alphaRectangle.Top)
					{
						newPoint.Y = _alphaRectangle.Top;
					}
					else if (newPoint.Y > _alphaRectangle.Bottom)
					{
						newPoint.Y = _alphaRectangle.Bottom;
					}
					newAlphaPoint = new Point(_alphaRectangle.Right, newPoint.Y);
					double alphaScale = (double)255 / (double)(_alphaRectangle.Height);
					_alpha = (byte)((_alphaRectangle.Bottom - newPoint.Y) * alphaScale);
					break;
		...
		private Point CalcAlphaPoint()
		{
			double alphaScale = (double)255 / (double)(_alphaRectangle.Height);
			return new Point(alphaPoint.X, (int)(_alphaRectangle.Bottom - (int)_alpha / alphaScale));
		}
		...
		private void UpdateDisplay()
		{
			...
			using (Brush selectedBrush = new SolidBrush(Color.FromArgb(_alpha, selectedColor)))
			{
				// Draw the saved color wheel image.
				g.DrawImage(colorImage, colorRectangle);

				// Draw the "selected color" rectangle.
                g.DrawImage(global::GorgonLibrary.Graphics.Tools.Properties.Resources.buttonBack, selectedColorRectangle);
		...
		private void CreateGradient()
		{
				...
				new PathGradientBrush(GetPoints(radius, new Point(radius, radius))))
			{
				...
				using (System.Drawing.Graphics newGraphics =
								 System.Drawing.Graphics.FromImage(colorImage))
				{
